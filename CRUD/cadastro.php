...
session_start();
include 'conexao.php';
$label_botao = "Cadastrar";
$link_cancelar = "login.php";
$row = array();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    $email_prefeitura = $_POST['email_prefeitura'];
    ...
    $email_responsavel_pela_inscricao = $_POST['email_responsavel_pela_inscricao'];
		...
		if (isset($_SESSION['cnpj_prefeitura']) && !empty($_SESSION['cnpj_prefeitura']) && isset($_POST['cnpj_editar']) && !empty($_POST['cnpj_editar']))
		{
			...
			$sql = "UPDATE pcm2025_prefeitura 
					SET 
						...
						email_responsavel_pela_inscricao = '$email_responsavel_pela_inscricao', 
						senha = '$senha', 
						data_alteracao = NOW() 
					WHERE cnpj_prefeitura = '$cnpj_editar'";
		} 
		else // SE NÃO FOR ALTERAÇÃO
		{
			...
			{
				$erro = "S";
				echo "<div class='text-center alert alert-danger'>Erro, ja existe município ou CNPJ $cnpj_prefeitura cadastrado</div>";
			}
			else
			{
				// QUERY PARA INSERIR NOVO REGISTRO
				$sql = "INSERT INTO pcm2025_prefeitura (id_municipio, cnpj_prefeitura, email_prefeitura, fone_prefeitura, endereco_prefeitura, cep_prefeitura, nome_prefeito, cpf_prefeito, nome_responsavel_pela_inscricao, cpf_responsavel_pela_inscricao, cargo_responsavel_pela_inscricao, fone_responsavel_pela_inscricao, email_responsavel_pela_inscricao, senha, data_cadastro) 
				VALUES ('$id_municipio', '$cnpj_prefeitura', '$email_prefeitura', '$fone_prefeitura', '$endereco_prefeitura', '$cep_prefeitura', '$nome_prefeito', '$cpf_prefeito', '$nome_responsavel_pela_inscricao', '$cpf_responsavel_pela_inscricao', '$cargo_responsavel_pela_inscricao', '$fone_responsavel_pela_inscricao', '$email_responsavel_pela_inscricao', '$senha', NOW())";
				$cnpj_editar = $cnpj_prefeitura;
				//echo $sql;
			}
		
		}
		// SE NAO EXISTIR ERRO EXECUTA QUERY NO BANCO DE DADOS
		if($erro == "N")
		{
			if (mysqli_query($conn,$sql))  {
				echo "<div class='text-center alert alert-success'>Salvo com sucesso dados do CNPJ $cnpj_prefeitura</div>";
			} else {
...
if ((isset($_SESSION['cnpj_prefeitura']) && !empty($_SESSION['cnpj_prefeitura'])) || ((isset($_GET['cnpj']) && !empty($_GET['cnpj'])) || (isset($_POST['cnpj_editar']) && !empty($_POST['cnpj_editar']))))
{
	...
	if(isset($cnpj_editar))
	{
		...
		$sql = "SELECT p.id,
				...
				p.email_responsavel_pela_inscricao,
				p.data_alteracao, 
				m.municipio 
...
<div class="container-lg">
    ...
    <form method="POST" onsubmit="return validarSenha()">
        ...
        <label>Email:</label>
        <input type="email" name="email_prefeitura" class="form-control" value="<?php echo @$row['email_prefeitura'] ?>" required>
        ...
        <label>Cargo do Responsavel pela inscrição:</label>
        ...
        <label>Email:</label>
        <input type="email" name="email_responsavel_pela_inscricao" class="form-control" value="<?php echo @$row['email_responsavel_pela_inscricao'] ?>" required>
